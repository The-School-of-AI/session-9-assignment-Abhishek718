...
NO_OF_PROFILES = 100
fake_profiles = Assignment9.get_fake_profiles('namedtuple', NO_OF_PROFILES)
fake_profiles_dict = Assignment9.get_fake_profiles('dict', NO_OF_PROFILES)
...
def test_get_fake_profiles_named_tuple():
	assert type(Assignment9.get_average_age(fake_profiles,'namedtuple')) == float,"fake profile(namedtuple) does not get created"

def test_get_fake_profiles_named_dictionary():
	assert type(Assignment9.get_average_age(fake_profiles_dict,'dict')) == float,"fake profile(dictionary) does not get created"

def test_get_largest_blood_group():
	assert type(Assignment9.get_largest_blood_group(fake_profiles,'namedtuple')) == tuple,"did not give tuple of blood group with count"

def test_get_oldest_birthdate():
	assert str(type(Assignment9.get_oldest_birthdate(fake_profiles,'namedtuple'))) == "<class 'datetime.date'>","did not give date of birth"

def test_get_average_age():
	assert type(Assignment9.get_average_age(fake_profiles,'namedtuple')) == float,"did not give date of birth"
...
def test_get_average_time():
	...
	n = Assignment9.get_average_time(1000,add,10,35)
